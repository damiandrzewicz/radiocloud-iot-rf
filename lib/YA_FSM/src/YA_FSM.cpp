...
uint8_t YA_FSM::AddState(const char* name, uint32_t maxTime, uint32_t minTime,
				action_cb_proxy onEntering, action_cb onState, action_cb_proxy onLeaving){
...
uint8_t YA_FSM::AddState(const char* name, uint32_t maxTime, action_cb_proxy onEntering, action_cb onState, action_cb_proxy onLeaving){
	return AddState(name, maxTime, 0, onEntering, onState, onLeaving);
}

uint8_t YA_FSM::AddState(const char* name, action_cb_proxy onEntering, action_cb onState, action_cb_proxy onLeaving){
...
bool YA_FSM::Update(){
	...
	for(FSM_Transition* actualtr = _firstTransition; actualtr != nullptr; actualtr = actualtr->nextTransition) {
		//Log.verboseln("InputState=%d, index=%d", actualtr->InputState, _currentState->index);
		if(actualtr->InputState == _currentState->index){
			...
			if(actualtr->Condition == nullptr){
				//Log.verboseln("cond null");
				_trigger = *(actualtr->ConditionVar);
			}
			else{
				_trigger = actualtr->Condition(_currentState);
			}
			...
			if (_trigger){
				...
				if(_currentState->OnLeaving != nullptr){
					Log.verboseln(F("<-- %s"), _currentState->stateName);
					_currentState->OnLeaving();
				} else {
					//Log.verboseln("OnLeaving nullptr");
				}
				...
				_currentState->result = FSM_StateResult::Working;

				FSM_State* targetState = GetStateAt(actualtr->OutputState);
				Log.verboseln(F("fsm_tr: %s(res:%d, timeout:%d)->%s"), 
					_currentState->stateName, 
					static_cast<uint8_t>(_currentState->result), 
	...
	if(_currentState->OnState != nullptr){
		//Log.verboseln(F("-- %s --"), _currentState->stateName);
		_currentState->result = _currentState->OnState();
	} else {
		//Log.verboseln("OnState nullptr");
	}
...
void YA_FSM::SetOnEntering(uint8_t index, action_cb_proxy action){
	FSM_State* state = GetStateAt(index);
	state->OnEntering = action;
}

void YA_FSM::SetOnLeaving(uint8_t index, action_cb_proxy action){
	FSM_State* state = GetStateAt(index);
