...
#ifdef __AVR__
	typedef nonstd::function<bool(const FSM_State*)> condition_cb;
	typedef nonstd::function<FSM_StateResult()> action_cb;
	typedef nonstd::function<void()> action_cb_proxy;
#else
	typedef std::function<bool()> condition_cb;
	typedef std::function<FSM_StateResult()> action_cb;
#endif
...
struct FSM_State {
	...
	uint32_t 	enterTime;
	action_cb_proxy 	OnEntering;
	action_cb_proxy 	OnLeaving;
	action_cb 			OnState;
	const char 	*stateName;
	FSM_State	*nextState = nullptr;
	FSM_Action  *lastAction = nullptr;
	FSM_StateResult result = FSM_StateResult::Working;
} ;
...
public:
	...
	void 	SetOnEntering(uint8_t index, action_cb_proxy action);
	[[deprecated("Replaced by Timeout()")]]
	void 	SetOnLeaving(uint8_t index, action_cb_proxy action);
	[[deprecated("Replaced by Timeout()")]]
